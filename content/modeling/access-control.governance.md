...
## Governance and the SDK

Governance groups aren't limited to MCP users — SDK tokens can also carry group memberships. When exchanging a secret key for a token via `POST /api/sdk/token`, pass a `groups` array to restrict the token to specific governance groups:
...
The resulting token sees only what the "analysts" group can access (view visibility, field restrictions, row filters), plus any `group: sdk` policies for tenant isolation. All SDK tokens automatically include the `sdk` group. If `groups` is omitted, the token carries `["sdk"]` only.

See [sdk.authentication](sdk.authentication) for the full token exchange guide.
...
- **Developer-defined policies** — written in YAML, typically for B2B tenant isolation using `group: sdk` (matches SDK tokens only)
